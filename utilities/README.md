...
Le fichier `debug_downlink.js` permet de tester et visualiser l'encodage d'une commande downlink dans la console.  
...
```bash
    node ./debug_downlink.js <device> [<json_data>]
```

Par exemple:

```bash
    node ./debug_downlink.js "smartpilot_wire" "{\"sendMSOMode\":5}"
```
